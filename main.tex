\section{Introduction}
    ...
    inventory information in the U.S. from federal databases
    and national reports, with primary reference to the most recent U.S.\ Spent Nuclear Fuel
    and Reprocessing Waste Inventory Report published in 2023 \cite{carter_spent_2023}. While the underlying national inventory assessment
    was generated based on GC-859 \cite{GC-859} and UNF-ST\&DARDS database \cite{banerjee_unf-st&dards:_2016}, this summary is
    produced in 2025 and incorporates the updated data and isotopic information from the STANDARDS~5.0.1 database
    to support current fuel cycle modeling needs.\\

    The national inventory report provides both historical inventories and long-term projections of commercial and \gls{DOE}-managed
    spent nuclear fuel and \gls{HLW} extending through 2075. These projections capture the expected evolution of discharged
    fuel mass, burnup, isotopic composition, storage configuration, and decay heat over multiple decades, making the dataset particularly
    valuable for evaluating advanced fuel cycle and waste management strategies.\\

    This summary serves to inform isotopic 
    ...
    \gls{EINSTEIN} project \cite{fang_einstein_2025}.
...
\section{Geospatial Information}
    ...
    data from the datasets coupled with 
    ...
    Notable concentrations of \gls{SNF} storage sites are observed in the northeastern United States,
    the Midwest, the Southeast, and along the West Coast, reflecting the historical deployment of the nation's light water reactor fleet.
    This spatial distribution is an important consideration for future fuel consolidation strategies, transportation planning, and
    repository siting analyses.

    \begin{figure}[H]
        \begin{center}
            \includegraphics[width=\textwidth]{figures/US_map.png}
        \end{center}
        \caption{Locations of sites storing spent nuclear fuel and reprocessing waste in the United States as of the end of 2022. \cite{carter_spent_2023}}
        \label{fig1}
    \end{figure}
    \todo{Caption of Figure 1 is not a complete sentence.}
    ...
    \begin{table}[H]
        ...
        \caption{U.S. \gls{SNF} and Reprocessing Waste Inventory Summary as of 
            December 31, 2022 \cite{carter_spent_2023}.}
        \label{tab1}
        \vspace{0.3em}
        \footnotesize
        \textit{Table notes:}
...
\section{U.S. Spent Nuclear Fuel and High-Level Waste Inventory}
\subsection{Commercial \gls{SNF} Inventory} 
 ...
 the \gls{INL}. Of the remaining fleet, 121 are \glspl{LWR}. Nearly every plant hosts a co-located \gls{ISFSI}, which becomes the primary
 storage facility after plant decommissioning.\\
    ...
    enrichment, and burnup. As of December 2022,
    ...
    \begin{table}[H]
        ...
        \end{center}
            \caption{Current Inventory at \gls{NPP} site by Storage Method as of 
            December 2022 \cite{carter_spent_2023}}
        \label{tab3}
    \end{table}
    ...
    \begin{figure}[H]
        ...
        \end{center}
            \caption{Commercial \glspl{NPP} and non-\gls{DOE} \glspl{ISFSI} currently store spent nuclear fuel throughout the United States \cite{carter_spent_2023}.}
        \label{fig2}
    \end{figure}
    ...
    \begin{figure}[H]
        ...
        \end{center}
        \caption{Dry spent nuclear fuel storage casks are currently loaded at nuclear power reactor sites across the United States \cite{carter_spent_2023}.}
        \label{fig3}
    \end{figure}
    ...
    risen from about 2\% $^{235}U$. In early fuel designs to nearly 4.5\% in modern \gls{PWR} and \gls{BWR} assemblies. These changes reflect
    improvements in core design, regulatory allowances, and utility strategies aimed at extending fuel cycle lengths and
    improving economic performance\cite{sassani_-line_2016}.\\
    ...
    This same concept was foundational to earlier Cyclus-based inventory modeling. In the UNF-ST\&DARDS-driven work by \textit{Bae et al.} (2020)\cite{bae_deep_2020},
    the burnup–enrichment distribution from the \gls{UDB} was used as the training domain for a neural-network depletion model that predicted
    isotopic compositions for every assembly in the U.S. inventory, and that model was then embedded into Cyclus as a reactor archetype
    capable of handling assembly-level variability.\\
    
    In this work, STANDARDS 5.0.1 provides the modern, expanded equivalent of the UDB
    \todo{UDB should be written using glossaries.}%
    used by \textit{Bae et al.}
    The distribution shown in Figure~\ref{fig4} is therefore not merely descriptive: it defines the domain over which isotopic
    vectors are extracted and mapped into Cyclus inventory and \gls{XDS} facility inputs.\\
    ...
    \begin{figure}[H]
        ...
        \end{center}
            \caption{Burnup (GWd/MTHM) and initial enrichment (\% $^{235}U$) distributions for spent nuclear fuel assemblies in the \glspl{LWR} inventory through December 2022 \cite{carter_spent_2023}.}
        \label{fig4}
    \end{figure}
    \todo{Caption of Figure 4 is not a complete sentence.}

    \begin{table}[H]
        \begin{center}
            \includegraphics[width=\textwidth]{figures/pwr_table.png}
        \end{center}
            \caption{PWR 60 GWd/MT Spent Nuclear Fuel Decay Heat \cite{carter_spent_2023}.}
        \label{tab4}
    \end{table}
    ...
    \begin{figure}[H]
        ...
        \end{center}
        \caption{Trends in average annual burnup (GWd/MT) and initial enrichment (\% $^{235}$U) of U.S. \glspl{LWR} spent nuclear fuel assemblies by discharge year. \cite{carter_spent_2023}.}
        \label{fig5}
    \end{figure}
    \todo{Caption of Figure 5 is not a sentence.}
    ...
    \begin{figure}[H]
        ...
        \end{center}
        \caption{Decay heat (W/MT) as a function of cooling time for \gls{PWR} fuel discharged at 60~GWd/MT and cooled for 1~year, including contributions from major radionuclide groups and the total decay heat \cite{carter_spent_2023}.}
        \label{fig6}
    \end{figure}
    \todo{Caption of Figure 6 is not a complete sentence.}

    Representative burnup cases provide insight into decay heat behavior. 4T4BE46K39R100613
    \todo{Are these numbers/letters intentional?}%
    ...
    cycle modeling tools such as \textsc{Cyclus}, ORIGEN, and UNF-ST\&DARDS.\\
    ...
    dominated by fission products such as $^{137}Cs$ and $^{90}Sr$. At longer cooling times, actinides, particularly $^{238}Pu$, $^{241}Pu$,
    and $^{241}Am$, become the primary contributors. Higher burnup fuel retains more long-term decay heat, which has implications
    ...
    \begin{table}[H]
        ...
        \end{center}
        \caption{BWR 50 GWd/MT Spent Nuclear Fuel Decay Heat \cite{carter_spent_2023}.}
        \label{tab5}
    \end{table}
    ...
    \begin{figure}[H]
        ...
        \end{center}
        \caption{Decay heat (W/MT) as a function of cooling time for \gls{BWR} fuel discharged at 50~GWd/MT and cooled for 1~year, including contributions from major radionuclide groups and the total decay heat \cite{carter_spent_2023}.}
        \label{fig7}
    \end{figure}
    \todo{Caption for Figure 7 is not a complete sentence.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\gls{SNF} at \gls{DOE} Locations}
    ...
    heavy water reactors, TRIGA 
    \todo{TRIGA should be written using glossaries}%
    research reactors, breeder reactors, and early prototype power reactors. Fuel enrichments
    span a wide range from depleted uranium for production reactors to highly enriched uranium exceeding 93\% $^{235}U$ for
    ...
    challenges in characterization, consolidation, and long term disposal planning.\\
     ...
     By the end of 2022, \gls{DOE} managed approximately 2,446~MTHM
     \todo{MTHM should be written using glossaries}%
     of \gls{SNF} (excluding naval fuel), as documented in the GC-859 database.
    ...
    \gls{DOE} \gls{SNF} is relatively low power at long cooling times\cite{sassani_-line_2016}.


    \begin{table}[H]
        \begin{center}
            \includegraphics[width=0.65\textwidth]{figures/doe_projection.png}
        \end{center}
        \caption{Projected decay heat distribution of \gls{DOE} spent nuclear fuel canisters for the year 2030 \cite{carter_spent_2023}.}
        \label{tab6}
        \vspace{0.3em}
        \footnotesize
        \textit{Table note:}
        \textsuperscript{11} Canister counts are derived from an SFD
	    \todo{SFD needs to be written using glossaries.}%
	    loading algorithm and have been rounded up to whole canisters. They provide a relative comparison across decay-heat ranges and do not represent actual as-loaded canisters. Totals may therefore differ from the SFD 
	    \todo{SFD needs to be written using glossaries}%
	    database, and cumulative percentages are based on the algorithm results.
        \normalsize
    \end{table}
    ...
    all \gls{DOE}-managed \gls{SNF}. Hanford contains the largest share, approximately 2,128~MTHM
    \todo{MTHM should be written using glossaries}%
    , primarily stored in sealed stainless steel
    ...
    \gls{SRS} holds approximately 27~MTHM
    \todo{MTHM should be written using glossaries}%
    , largely from isotope production reactors and research reactor returns. Although \gls{DOE} has
    ...
    into these standardized canisters\cite{arthur_accelerator-based_1995}\\

    \gls{DOE} also manages approximately 173.6~MTHM
    \todo{MTHM should be written using glossaries.}%
    of spent nuclear fuel that originated from commercial nuclear power reactors,
    ...
    \begin{table}[H]
        \begin{center}
            \includegraphics[width=0.7\textwidth]{figures/doe.png}
        \end{center}
        \caption{Decay heat characteristics of canisters containing \gls{SNF} of commercial (NPR) origin managed by \gls{DOE} \cite{carter_spent_2023}.}
        \label{tab7}
        \vspace{0.3em}
        \footnotesize
        \textit{Table note:}
    ...
    \begin{table}[H]
        \begin{center}
            \includegraphics[width=0.75\textwidth]{figures/naval.png}
        \end{center}
        \caption{Decay heat characteristics of naval \gls{SNF} canisters \cite{carter_spent_2023}.}
        \label{tab8}
    \end{table}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\gls{SNF} at Other Sites}
    ...
    heavy metal (MTHM)
    \todo{Here MTHM is defined for the first time, but it is used many times before this instance. Either define sooner, or no need to redefine here}%
    , yet they represent a diverse set of institutions and reactor types that contribute to the broader national
    ...
    such as the AGN, 201 \todo{AGN, 201 should be AGN-201}%
    reactors or the Rensselaer Polytechnic Institute Critical Facility, operate at such low power that they are
    ...
    \begin{table}[H]
        ...
        \end{center}
        \caption{University research reactor sites and their associated \gls{SNF} inventories \cite{carter_spent_2023}.}
        \label{tab9}
    \end{table}
    ...
    \begin{table}[H]
        \begin{center}
            \includegraphics[width=0.8\textwidth]{figures/government.png}
        \end{center}
        \caption{Research reactors operated by other government agencies and the associated \gls{SNF} transferred to \gls{DOE} custody for long term management \cite{carter_spent_2023}.}
        \label{tab10}
    \end{table}
    ...
    \begin{table}[H]
        \begin{center}
            \includegraphics[width=0.8\textwidth]{figures/commercial_research.png}
        \end{center}
        \caption{Commercial research and development centers with spent nuclear fuel inventories \cite{carter_spent_2023}.}
        \label{tab11}
    \end{table}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\gls{SNF} Reprocessing Inventory}
    ...
    fast reactor programs\cite{aliberti_nuclear_2006}.\\
    ...
    continues, the radioactive content of the waste will decrease only through radioactive decay\cite{carter_spent_2023}.\\
...
\section{Incorporating the National \gls{SNF} Inventory into \textsc{Cyclus}}
    ...
    \textsc{Cyclus} provides a physics-informed, time-dependent simulation environment for modeling complex nuclear fuel cycles with
    isotopic resolution. Unlike static material flow models, \textsc{Cyclus} explicitly tracks the movement, transformation, decay,
    and accumulation of nuclear materials through user-defined facilities over time. Each material object contains a full nuclide vector,
    allowing transuranics, fission products, and decay chains to be propagated consistently throughout the simulation\cite{gidden_agent-based_2015}.\\

    A key strength of \textsc{Cyclus} is its agent-based architecture, in which reactors, separation plants, storage facilities, repositories,
    and fuel cycle technologies are represented as independent but interacting agents. These agents exchange material through market-like
    request and bid mechanisms, enabling realistic modeling of supply constraints, technology deployment schedules, and competition for limited
    resources. This makes \textsc{Cyclus} particularly well suited for analyzing scenarios in which advanced systems such as \gls{XDS} draw
    selectively from a heterogeneous national spent fuel inventory\cite{scopatz_cyclus_2015}.\\

    \textsc{Cyclus} natively supports isotopic decay, mass tracking, and transmutation through its physics kernel and material composition
    libraries. It can model multiple fuel forms, burnup-dependent isotopic vectors, and time-dependent cooling, all of which are essential
    for evaluating decay heat, radiotoxicity, and reactivity in advanced transmutation systems. Furthermore, \textsc{Cyclus} allows facilities
    to implement custom physics models for irradiation, separation, and waste conditioning, making it possible to represent accelerator-driven
    systems, molten-salt reactors, and reprocessing technologies within a single, integrated framework.\\

    These capabilities allow the national \gls{SNF} inventory characterized in this report to be used not only as a static dataset, but as a
    dynamically evolving resource that can be evaluated under competing deployment strategies. By combining STANDARDS-derived isotopic data
    with \textsc{Cyclus}’s agent-based and physics-aware modeling infrastructure, this work enables quantitative assessment of how \gls{XDS}
    technologies could alter the long-term trajectory of the U.S. nuclear waste inventory\cite{baeFuelCycleTransition2018}.
    ...
    \subsection{U.S. Inventory module}
    A central objective of this work is to quantify the fuel cycle performance of \gls{XDS} technologies at isotopic
    resolution. Achieving this requires input streams that reflect the true burnup, enrichment, and decay history of
    spent fuel discharged from U.S. reactors. Using the updated STANDARDS 5.0.1 dataset, age-corrected isotopic vectors
    are constructed for representative \gls{SNF} spanning a wide range of operating and cooling conditions. These data
    allow the national inventory to be decomposed into resource classes with different transuranic content,
    radiotoxicity, and transmutation potential, enabling \textsc{Cyclus} simulations to distinguish which portions of the
    inventory are most suitable for \gls{XDS} deployment\cite{peterson_additional_2017}.\\
    ...
    % Three vertical dots (centered between 3rd and last arrow)
    \node at ($(inv.east)+(0.5*\arrowlen,\yone-2.5*\dy)$)
        {
            \begin{tikzpicture}
                \draw[line width=1.1pt, teal!85!black] (0,0.19) -- (0,0.11);
                \draw[line width=1.1pt, teal!80!black] (0,0.05) -- (0,-0.03);
                \draw[line width=1.1pt, teal!70!black] (0,-0.10) -- (0,-0.18);
            \end{tikzpicture}
        };
    ...
    \caption{Structure of the U.S. Inventory module based on STANDARDS 5.0.1 dataset, showing representative PWR and BWR assemblies defined by initial enrichment, burnup, and isotopic composition. 
    The module holds all the assemblies in the national spent fuel inventory.}
    \end{figure}

    To support this functionality, a dedicated U.S.\ inventory module is being developed within \textsc{Cyclus}.
    This module represents the national waste inventory as a dynamic storage facility populated with \gls{SNF} batches parameterized
    using STANDARDS data. Each batch is stored as a \textsc{Cyclus} material object with associated mass and isotopic composition.
    Radioactive decay can be handled either by initializing materials with age-corrected compositions derived from STANDARDS or
    by allowing the built-in \textsc{Cyclus} decay mechanisms to evolve isotopics during the simulation.
    External facilities, such as \gls{ADS} or \gls{FDS} modules, can request material according to user-defined criteria.
    When material is transferred, the inventory is automatically updated, enabling tracking of consumption rates, remaining resources,
    and time-dependent evolution of the waste stockpile.


    \subsection{\gls{XDS} Facility Modules}

    Parallel to the inventory module, an \gls{XDS} facility module is under development to generically represent accelerator driven systems
    ...
    units are needed to achieve specific waste reduction goals, and how quickly the inventory could be transmuted under different scenarios\cite{herrera-martinez_transmutation_2007}.
    \todo{Explanation of XDS system design does not cite Wisconsin design.}
    ...
    inputarrow/.style={->, line width=1pt, ForestGreen!70!black},
    actarrow/.style={->, line width=1pt, BrickRed!80!black},
    llfparrow/.style={->, line width=1pt, NavyBlue!80!black},
    fparrow/.style={->, line width=1pt, Goldenrod!80!black},
    wastearrow/.style={->, line width=1pt, darkgray!80}
    ]
    ...
    \caption{Structure of the \gls{XDS} facility archetype within the Cyclus framework, showing the input material feed and the resulting output commodity streams for residual actinides, LLFPs, and other fission products, along with the waste stream.}
    \end{figure}
    \todo{Figure caption is not a complete sentence.}


\subsection{Simulation architecture in \textsc{Cyclus}}
    ...
    the commercial and \gls{DOE}-managed \gls{SNF} resources, enabling robust evaluation of advanced fuel cycle strategies\cite{carlsen_cycamore_2014}.\\
...
\begin{tikzpicture}[
    >=Stealth,
    font=\large,                        % Bigger overall font
    node distance=3.2cm,
    every path/.style={line width=1.4pt}, % Thicker arrows
    block/.style={
        draw,
        rounded corners,
        minimum width=3.9cm,
        minimum height=2.4cm,
        line width=1.4pt,                % Thicker box borders
        align=center
    },
    blueblock/.style={block, fill=blue!15},
    pinkblock/.style={block, fill=red!10},
    annot/.style={font=\normalsize, align=center} % Bigger arrow labels
]
...
\node[blueblock, right=of mix] (xds) {XDS\\Facilities};

%-------------------- NODES (bottom row) --------------------%
\node[pinkblock, below=6.0cm of sep] (stor_sep) {Storage\\Fcaility};
\node[pinkblock, below=6.0cm of xds] (stor_xds) {XDS Storage\\ Facility};

%-------------------- LEGEND --------------------%
\node[anchor=west] (legend) at ([xshift=20.2cm,yshift=-4.0cm]inv.south east) {%
  \begin{tikzpicture}[arrows=-, line width=0.8pt]
    \node[draw, rounded corners=6pt, fill=blue!15,
          minimum width=1.6cm, minimum height=0.75cm] (l1) {};
    \node[anchor=west, font=\normalsize] at ([xshift=0.35cm]l1.east)
          {Developing archetypes};

    \node[draw, rounded corners=6pt, fill=red!10,
          minimum width=1.6cm, minimum height=0.85cm] (l2)
          at ([yshift=-0.85cm]l1.south west) {};
    \node[anchor=west, font=\normalsize]
          at ([xshift=0.35cm]l2.east)
          {Existing Cycamore archetypes};
  \end{tikzpicture}%
};
...
\draw[->] (sep.east) ++(0,0.7) -- ([yshift=0.7cm]mix.west)
    node[midway, above, annot]{MAs};

\draw[->] (sep.east) -- (mix.west)
    node[midway, above, annot]{LLFPs};

\draw[->] ([yshift=-0.7cm]sep.east) -- ([yshift=-0.7cm]mix.west)
  node[midway, above]{
    \begin{tikzpicture}[scale=0.8, arrows=-]
      \draw[line width=1.0pt] (0,0.62) -- (0,0.55);
      \draw[line width=1.0pt] (0,0.50) -- (0,0.43);
      \draw[line width=1.0pt] (0,0.38) -- (0,0.31);
    \end{tikzpicture}
  };

\draw[->] (mix) -- node[above, annot]{Material Feed} (xds);

\draw[->] (sep.south) -- (stor_sep.north)
    node[midway, right, annot]{unwanted\\composition};

\draw[->] (xds.south) -- (stor_xds.north)
    node[midway, left, annot]{XDS\\ Waste};

\draw[->] (xds.east) ++(0,0.7) -- ++(5,0)
...
\caption{Conceptual flow from STANDARDS5.0.1 inventory to \gls{XDS} and separate storage pathways for separation waste and XDS waste.
Blue facilities denote proposed facilities for this work, while pink facilities represent existing \textsc{Cycamore} facilities.}
\end{minipage}
}
\end{figure}
    ...
    and neutron absorption in the commercial \gls{SNF} inventory: uranium ($^{235}U$, $^{238}U$), plutonium ($^{238}Pu$ through $^{242}Pu$), neptunium-237,
    americium ($^{241}Am$, $^{242m}Am$, $^{243}Am$), and curium ($^{244}Cm$ through $^{247}Cm$). Plutonium isotopes are included to maintain the subcritical multiplication
    factor of the \gls{XDS}, while the MA 
    \todo{MA should be written using glossaries}%
    vector represents the primary target for transmutation. In some \gls{XDS}s designs, technetium-99 and iodine-129 are
    ...
    In this scenario the MA 
    \todo{MA should be written using glossaries.}%
    composition described above is supplemented with selected LLFP 
    \todo{LLFP should be written using glossaries}%
    isotopes that significantly influence long term radiotoxicity or
    neutron economy, including $^{99}Tc$, $^{129}I$, $^{135}Cs$, $^{79}Se$, $^{126}Sn$, $^{93}Zr$, $^{107}Pd$, $^{85}Kr$, and $^{151}Sm$.
    These LLFPs are incorporated only when the system design explicitly targets LLFP 
    \todo{Both instances of LLFP should be written using glossaries}%
    destruction;
    ...
    that occur during transmutation. Following irradiation, the material contains reduced quantities of minor actinides, particularly $^{241}Am$, $^{243}Am$, $^{237}Np$,
    and multiple curium isotopes, which reflects the primary transmutation objective of the system.
    The plutonium vector evolves through a combination of fission and neutron capture reactions, typically showing reduced $^{239}Pu$ and $^{241}Pu$, and corresponding growth in $^{240}Pu$ and $^{242}Pu$.
    When LLFP 
    \todo{LLFP should be written uisng glossaries}%
    transmutation is not activated in the scenario, long lived isotopes such as $^{99}Tc$, $^{129}I$, $^{126}Sn$, and $^{107}Pd$ remain in the output inventory.
    ...
    providing a physically realistic representation of \gls{XDS} discharge material for further processing, storage, or reinsertion into subsequent fuel cycle stages\cite{wagner_categorization_2013}.
