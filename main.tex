...
\section{Incorporating the National \gls{SNF} Inventory into \textsc{Cyclus}}
    ...
    allowing transuranics, fission products, and decay chains to be propagated consistently throughout the simulation.\\
    ...
    selectively from a heterogeneous national spent fuel inventory.\\
    ...
    technologies could alter the long-term trajectory of the U.S. nuclear waste inventory.
    ...
    inventory are most suitable for \gls{XDS} deployment.\\
    ...
    % Main box
    \node[box] (inv) {\LARGE U.S. Inventory\\[2mm]\LARGE STANDARDS 5.0.1};

    % Output arrow + label 
    \draw[->, line width=0.8pt] (inv.east) ++(0,+0.75) -- ++(6,0)
    node[midway, above, font=\small] {PWR, Init\_Enr=3.8\%, BU=41919};
    
    \draw[->, line width=0.8pt] (inv.east) ++(0,+0.25) -- ++(6,0)
    node[midway, above, font=\small] {PWR, Init\_Enr=4.2\%, BU=43597};

    \draw[->, line width=0.8pt] (inv.east) ++(0,-0.25) -- ++(6,0)
    node[midway, above, font=\small] {BWR, Init\_Enr=2.19\%, BU=6237};

    \draw[->, line width=0.8pt] (inv.east) ++(0,-0.75) -- ++(6,0)
    node[midway, above, font=\small] {$\dots\ N$ assemblies};

    % % Input arrow + label
    % \draw[->, line width=0.9pt] ($(inv.west)+(-6,0)$) -- (inv.west)
    % node[midway, above] {\LARGE STANDARDS 5.0.1};

    \end{tikzpicture}
    }
    \caption{U.S. Inventory module}
    \end{figure}
    ...
    % Input arrow
    \draw[arrow] ($(xds.west)+(-4,0)$) -- (xds.west)
        node[midway, above] {\Large Material Feed};

    % ===== Output arrows (EDIT LABELS HERE) =====

    % Top-right output
    \draw[arrow] (xds.east) ++(0,0.7) -- ++(5,0)
        node[midway, above] {\Large Residual Actinides};

    % Middle-right output
    \draw[arrow] (xds.east) -- ++(5,0)
        node[midway, above] {\Large LLFP};

    % Bottom-right output
    \draw[arrow] (xds.east) ++(0,-0.7) -- ++(5,0)
        node[midway, above] {\Large Other FP};

    % Bottom output (optional extra channel)
    \draw[arrow] (xds.south) -- ++(0,-2)
        node[midway, left] {\Large XDS}
        node[midway, right] {\Large Waste};

    \end{tikzpicture}
    }
    \caption{\gls{XDS} Archetype design in Cyclus.}
    \end{figure}
...
%-------------------- NODES (top row) --------------------%
\node[blueblock] (inv) {U.S. Inventory\\STANDARDS 5.0.1};
...
\draw[->] (xds.east) ++(0,-0.5) -- ++(5,0)
    node[midway, above, annot]{Other FsP};
...
\end{figure}
    ...
    providing a physically realistic representation of \gls{XDS} discharge material for further processing, storage, or reinsertion into subsequent fuel cycle stages.~\cite{?}
