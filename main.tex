...
\section{Incorporating the National \gls{SNF} Inventory into \textsc{Cyclus}}
    ...
    % Main box
    \node[box] (inv) {\LARGE U.S. Inventory\\[2mm]\LARGE Archetype};

    % Output arrow + label (top)
    \draw[->, line width=0.8pt] (inv.east) -- ++(6.5,0)
    node[midway, above] {\LARGE Assemblies};

    % Output label (bottom)
    \node[anchor=west] at ($(inv.east)+(1.5,-0.65)$) {\LARGE\& compositions};
    ...
    %     node[midway, right] {\Large Waste streams};
...
\caption{Conceptual flow from STANDARDS5.0.1 inventory to \gls{XDS}, storage, and reprocessing.}
\end{figure}
